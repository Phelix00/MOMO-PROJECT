    ...
    while (run) {
		...
		if (authenticated){
			...
			if (option==0){
				...
				string number;
				float amount;
				cout << "ENTER THE NUMBER YOU WANT TO TRANSFER MONEY TO : "<<endl;
				cin>>number;
				cout << "ENTER THE AMOUNT YOU WANT TO SEND TO : "<<endl;
				cin>>amount;
				if (amount > balance) {
					cout << "YOUR ACCOUNT BALANCE IS NOT ENOUGH FOR THIS TRANSACTION."<<endl;
                }else {
                    balance -= amount;
